...
def setup_test_variables():
    ...
    params['src'] = torch.randint(
        1, params['src_vocab_size'], (params['batch_size'], params['src_len']))
    params['tgt'] = torch.randint(
        1, params['tgt_vocab_size'], (params['batch_size'], params['tgt_len']))
    params['src'][0, -2:] = params['pad_idx']
    params['tgt'][1, -1:] = params['pad_idx']

    device = params['src'].device
    params['src_mask'] = Transformer.create_padding_mask(
        params['src'], params['pad_idx'])
    tgt_padding_mask = Transformer.create_padding_mask(
        params['tgt'], params['pad_idx'])
...
def test_model_forward_shape(params):
    ...
    model.eval()
    with torch.no_grad():
        output = model(params['src_ids'], params['tgt_ids'],
                       params['src_mask'], params['tgt_mask'])

    expected_shape = (params['batch_size'],
                      params['tgt_len'], params['tgt_vocab_size'])
...
def test_causal_mask_logic(params):
    ...
    model = Transformer(
        ...
        d_model=params['d_model'], num_layers=params['num_layers'], n_heads=params['n_heads'],
        # 关闭 dropout
        d_ff=params['d_ff'], max_len=params['max_len'], dropout=0.0
    )
    model.eval()

    with torch.no_grad():
        output_base = model(
            params['src_ids'], params['tgt_ids'], params['src_mask'], params['tgt_mask'])

        tgt_modified = params['tgt_ids'].clone()
        new_token_val = (params['tgt_ids'][0, 5] +
                         10) % params['tgt_vocab_size']
        if new_token_val == params['pad_idx']:
            new_token_val += 1
        tgt_modified[0, 5] = new_token_val

        output_modified = model(
            params['src_ids'], tgt_modified, params['src_mask'], params['tgt_mask'])
