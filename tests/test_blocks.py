...
def setup_test_variables():
    ...
    params['src'] = torch.randn(
        params['batch_size'], params['src_len'], params['d_model'])
    params['src_mask'] = torch.ones(
        params['batch_size'], 1, 1, params['src_len'], dtype=torch.bool)
    params['src_mask'][:, :, :, -2:] = False

    params['tgt'] = torch.randn(
        params['batch_size'], params['tgt_len'], params['d_model'])
    params['enc_output'] = torch.randn(
        params['batch_size'], params['src_len'], params['d_model'])

    tgt_padding_mask = torch.ones(
        params['batch_size'], 1, params['tgt_len'], 1, dtype=torch.bool)
    tgt_padding_mask[:, :, -1, :] = False
    tgt_causal_mask = torch.tril(torch.ones(
        params['tgt_len'], params['tgt_len'], dtype=torch.bool))
...
def test_encoder_block_forward_shape(params):
    ...
    encoder_block = EncoderBlock(
        d_model=params['d_model'], n_heads=params['n_heads'], d_ff=params['d_ff'], dropout=params['dropout'])
    output = encoder_block(params['src_embed'], params['src_mask'])
    assert output.shape == params[
        'src_embed'].shape, f"Expected shape {params['src_embed'].shape}, but got {output.shape}"
...
def test_decoder_block_forward_shape(params):
    ...
    output = decoder_block(
        params['tgt_embed'], params['enc_output'], params['tgt_mask'], params['src_mask'])
    assert output.shape == params[
        'tgt_embed'].shape, f"Expected shape {params['tgt_embed'].shape}, but got {output.shape}"
...
def test_decoder_block_cross_attention_dependency(params):
    ...
    decoder_block = DecoderBlock(
        d_model=params['d_model'], n_heads=params['n_heads'], d_ff=params['d_ff'], dropout=0.0)
    decoder_block.eval()

    with torch.no_grad():
        output_base = decoder_block(
            params['tgt_embed'], params['enc_output'], params['tgt_mask'], params['src_mask'])
        enc_output_zero = torch.zeros_like(params['enc_output'])
        output_zero_enc = decoder_block(
            params['tgt_embed'], enc_output_zero, params['tgt_mask'], params['src_mask'])

    assert not torch.allclose(
        output_base, output_zero_enc), "Decoder output should change when encoder output changes."
