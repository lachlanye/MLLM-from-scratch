...
class MLLM(nn.Module):
    ...
    def forward(self, images: torch.Tensor, text_tokens: torch.Tensor):
        ...
        inputs_embeddings = torch.cat(
            [visual_embeddings, text_embeddings], dim=1)
    ...
    def generate(self, image: torch.Tensor, prompt: str, max_new_tokens: int, temperature: float = 0.7, top_k: int = None):
        ...
        visual_embeddings = self.connector(
            visual_features)  # Shape: [1, N_visual, D_lang]
        ...
        prompt_tokens = torch.tensor(
            initial_tokens, dtype=torch.long, device=device).unsqueeze(0)
        prompt_embeddings = self.language_model.token_embedding(
            prompt_tokens)  # Shape: [1, N_prompt, D_lang]

        # 3. 拼接视觉和文本嵌入作为初始输入 (已完成)
        input_embeddings = torch.cat(
            [visual_embeddings, prompt_embeddings], dim=1)
        ...
        for _ in range(max_new_tokens):
            ...
            if top_k is not None:
                v, _ = torch.topk(next_token_logits, min(
            ...
            #    - 这比每次都重新编码整个序列要高效得多。
            next_token_embedding = self.language_model.token_embedding(
                next_token)
            input_embeddings = torch.cat(
                [input_embeddings, next_token_embedding], dim=1)
