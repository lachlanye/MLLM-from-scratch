...
def predict(config: dict):
    ...
    try:
        model = ViT(img_size=data_cfg['img_size'], patch_size=model_cfg['patch_size'], in_channels=data_cfg['in_channels'], num_classes=data_cfg['num_classes'],
                    d_model=model_cfg['d_model'], num_layers=model_cfg['num_layers'], n_heads=model_cfg['n_heads'], d_ff=model_cfg['d_ff'], dropout=model_cfg['dropout']).to(device)
        model.load_state_dict(torch.load(model_path, map_location=device))
        model.eval()
        print(f"Model loaded successfully from '{model_path}'.")
    except FileNotFoundError:
        print(
            f"Error: Model file not found at '{model_path}'. Please run training first.")
        return
    ...
    class_names = data_cfg.get('class_names', None)
    if class_names is not None and len(class_names) > predicted_idx.item():
