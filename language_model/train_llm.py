...
def train(config: dict):
    ...
    full_dataset = TinyShakespeareDataset(
        root=data_cfg['data_dir'], block_size=data_cfg['block_size'], download=True)
    ...
    model = GPTModel(vocab_size=model_cfg['vocab_size'], d_model=model_cfg['d_model'], num_layers=model_cfg['num_layers'],
                     n_heads=model_cfg['n_heads'], d_ff=model_cfg['d_ff'], max_len=data_cfg['block_size'], dropout=model_cfg['dropout']).to(device)
    ...
    for epoch in range(train_cfg['num_epochs']):
        ...
        if (epoch + 1) % train_cfg['eval_interval'] == 0:
            ...
            with torch.no_grad():
                eval_loop = tqdm(
                    val_loader, leave=True, desc=f"Epoch {epoch+1}/{train_cfg['num_epochs']} [Eval]")
            ...
            generated_text = generate_sample_text(model=model, tokenizer=full_dataset.tokenizer, device=device,
                                                  start_string=start_prompt, max_new_tokens=150, block_size=data_cfg['block_size'])
