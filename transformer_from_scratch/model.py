...
class Transformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder = TransformerEncoder(
            num_layers=num_layers, d_model=d_model, n_heads=n_heads, d_ff=d_ff, dropout=dropout)
    ...
    def forward(self, src: torch.Tensor, tgt: torch.Tensor, src_mask: torch.Tensor, tgt_mask: torch.Tensor) -> torch.Tensor:
        ...
        #    - 将位置编码应用到经过缩放的嵌入上。
        src_processed = self.src_embedding(src) * math.sqrt(self.d_model)
        src_processed = self.pos_encoder(src_processed)
        tgt_processed = self.tgt_embedding(tgt) * math.sqrt(self.d_model)
        tgt_processed = self.pos_encoder(tgt_processed)
        ...
        dec_output = self.decoder(
            tgt_processed, enc_output, tgt_mask, src_mask)
