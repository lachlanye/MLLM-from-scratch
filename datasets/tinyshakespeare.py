    ...
    def __init__(self, root: str = "data", block_size: int = 128, download: bool = True, stride: int = None):
        ...
        if not os.path.exists(self.filepath):
            raise FileNotFoundError(
                f"Dataset file not found at {self.filepath}. Please enable download=True.")
        ...
        if os.path.exists(self.vocab_path):
            print(
                f"Found existing vocabulary at {self.vocab_path}. Loading...")
            self.tokenizer.load_vocab(self.vocab_path)
        else:
            print(
                f"No vocabulary found. Building from corpus and saving to {self.vocab_path}...")
        ...
        print(
            f"Corpus successfully encoded. Total characters (tokens): {len(self.data)}")
